...
{
    class TextMessage : MyEvents
    {


        public TextMessage(string name, string type, int ID, string Txt, double newLatitude, double newLongitude, string newDateTime) : base(name, type, ID, Txt, newLatitude, newLongitude, newDateTime)
        {

        }


        public TextMessage(XElement el)
        {
            EventName = el.Element("EventName").Value;
            EventID = int.Parse(el.Element("EventID").Value);
            EventType = "TextMessage";
            EventTxt = el.Element("TextMessage").Element("Description").Value;
            Latitude = double.Parse(el.Element("TextMessage").Element("Lat").Value);
            Longitude = double.Parse(el.Element("TextMessage").Element("Long").Value);
            DateTime = el.Element("TextMessage").Element("Datetimestamp").Value;
        }
        ...
        public override void CreateMarker(GMapControl gmap)
        {
            ...
            GMapMarker marker = new GMarkerGoogle(new PointLatLng(Latitude, Longitude), GMarkerGoogleType.lightblue_dot);
